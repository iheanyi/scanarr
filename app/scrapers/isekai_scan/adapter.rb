...
# IsekaiScan adapter (isekaiscan.top)
# Reference: https://github.com/keiyoushi/extensions-source (Madara multisrc)
#
# IsekaiScan is a WordPress/Madara-based manga aggregator focused on isekai content.
# Domain history: isekaiscan.com -> isekaiscan.to -> isekaiscan.top
#
...
module Scrapers
  ...
  class Adapter < Scrapers::BaseAdapter
    ...
    def browse(sort: "latest", page: 1, limit: 20)
      ...
      Rails.logger.error "[IsekaiScan] Browse error: #{e.message}"
      []
    end
    ...
    def search(query)
      ...
      Rails.logger.error "[IsekaiScan] Search error: #{e.message}"
      []
    end
    ...
    def series(id_or_url)
      ...
      Rails.logger.error "[IsekaiScan] Series error: #{e.message}"
      nil
    end

    # Fetch chapter list for a series
    # Madara sites load chapters via AJAX POST to admin-ajax.php
    ...
    def chapters(series_url)
      ...
      # First fetch the series page to get the manga ID for the AJAX call
      response = http.get(url)
      return [] unless response.status == 200

      doc = Nokogiri::HTML(response.body)

      # Try AJAX chapter loading first (standard Madara pattern)
      manga_id = extract_manga_id(doc, response.body)

      if manga_id
        chapters = load_chapters_via_ajax(manga_id)
        return chapters unless chapters.empty?
      end

      # Fallback: parse chapters from the page HTML
      parse_chapters(doc)
    rescue StandardError => e
      Rails.logger.error "[IsekaiScan] Chapters error: #{e.message}"
      []
    end
    ...
    def pages(chapter_url)
      ...
      Rails.logger.error "[IsekaiScan] Pages error: #{e.message}"
      []
    end
    ...
    def browse_html_fallback(sort, page)
      ...
      Rails.logger.error "[IsekaiScan] Browse HTML fallback error: #{e.message}"
      []
    end
    ...
    # Extract manga ID from the series page (used for AJAX chapter loading)
    def extract_manga_id(doc, body)
      # Try JavaScript variable: var mangaID = '1543';
      match = body.match(/var\s+mangaID\s*=\s*['"](\d+)['"]/)
      return match[1] if match

      # Try data attribute on the chapter container
      holder = doc.at_css("#manga-chapters-holder")
      holder&.[]("data-id")
    end

    # Load chapters via AJAX (the standard Madara approach)
    def load_chapters_via_ajax(manga_id)
      response = http.get("#{base_url}/ajax-list-chapter", params: { "mangaID" => manga_id })
      return [] unless response.status == 200 && response.body.present?

      doc = Nokogiri::HTML(response.body)
      parse_chapters(doc)
    rescue StandardError => e
      Rails.logger.error "[IsekaiScan] AJAX chapter load error: #{e.message}"
      []
    end
    ...
    # Parse chapters from Madara chapter list HTML
    ...
    def parse_pages(doc)
      ...
      doc.css("div.page-break img, li.blocks-gallery-item img, .reading-content img").each_with_index do |img, idx|
        src = img["data-src"]&.strip || img["src"]&.strip
    ...
    # Parse Madara date formats (MM/dd/yy, "Month dd, yyyy", or relative like "3 hours ago")
    def parse_madara_date(text)
      return nil unless text.present?

      # Try MM/dd/yy format
      Date.strptime(text, "%m/%d/%y").to_time
    rescue Date::Error, ArgumentError
      begin
        # Try "Month dd, yyyy" or "Mon dd, HH:MM" format
        Time.parse(text)
