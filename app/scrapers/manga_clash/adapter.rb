...
# MangaClash adapter (mangaclash.com -> toonclash.com)
# Reference: https://github.com/keiyoushi/extensions-source (MangaClash / Madara multisrc)
#
# MangaClash is a WordPress/Madara-based manga aggregator.
# Domain changed from mangaclash.com to toonclash.com in April 2024.
  ...
  module MangaClash
    class Adapter < Scrapers::BaseAdapter
      ...
      def browse(sort: "latest", page: 1, limit: 20)
        ...
        # If AJAX fails, fall back to HTML page scraping
        ...
        Rails.logger.error "[MangaClash] Browse error: #{e.message}"
        []
      end
      ...
      def search(query)
        ...
        Rails.logger.error "[MangaClash] Search error: #{e.message}"
        []
      end
      ...
      def series(id_or_url)
        ...
        Rails.logger.error "[MangaClash] Series error: #{e.message}"
        nil
      end
      ...
      def chapters(series_url)
        ...
        Rails.logger.error "[MangaClash] Chapters error: #{e.message}"
        []
      end
      ...
      def pages(chapter_url)
        ...
        Rails.logger.error "[MangaClash] Pages error: #{e.message}"
        []
      end
      ...
      def browse_html_fallback(sort, page)
        ...
        Rails.logger.error "[MangaClash] Browse HTML fallback error: #{e.message}"
        []
      end
      ...
      # Madara search uses div.c-tabs-item__content for each result
      ...
      def extract_description(doc)
        ...
        # Remove "Show more" buttons and clean up
      ...
      def extract_status(doc)
        # Madara status is typically in div.summary-content next to a "Status" heading
      ...
      def parse_pages(doc)
        ...
        # Primary: div.page-break img (standard Madara reader)
      ...
      # e.g., https://toonclash.com/manga/one-piece/ -> "one-piece"
      ...
      def extract_chapter_title(text)
        ...
        # Remove "Chapter XX" prefix and clean up
