class LibraryPathBuilder
  ...
  def base_path
    ...
    source = slug(@source&.slug.presence || @source&.key.presence).presence || "unknown-source"
  ...
  def chapter_path(chapter)
    ...
    volume_segment = normalize_numeric(chapter.volume&.volume_number).presence

    return [ base, "chapters", chapter_segment ].join("/") if volume_segment.blank?
