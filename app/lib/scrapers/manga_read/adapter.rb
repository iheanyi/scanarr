...
# MangaRead adapter (mangaread.org)
# Reference: https://github.com/keiyoushi/extensions-source (Madara multisrc)
#
# MangaRead is a WordPress/Madara-based manga aggregator.
...
module Scrapers
  ...
  class Adapter < Scrapers::BaseAdapter
    ...
    def browse(sort: "latest", page: 1, limit: 20, filters: {})
      ...
      Rails.logger.error "[MangaRead] Browse error: #{e.message}"
      []
    end
    ...
    def search(query, filters: {})
      ...
      Rails.logger.error "[MangaRead] Search error: #{e.message}"
      []
    end
    ...
    def series(id_or_url)
      ...
      Rails.logger.error "[MangaRead] Series error: #{e.message}"
      nil
    end

    # Fetch chapter list for a series
    # Tries AJAX endpoint first, falls back to inline chapters on series page
    ...
    def chapters(series_url)
      ...
      # Try AJAX chapter loading first (Madara uses admin-ajax.php with manga_get_chapters)
      ajax_response = http.post(
        "#{base_url}/wp-admin/admin-ajax.php",
        body: {
          "action" => "manga_get_chapters",
          "manga" => extract_post_id_from_url(url)
        },
        headers: xhr_headers
      )
      ...
      # Fallback: try /ajax/chapters/ endpoint
      ajax_url = url.chomp("/") + "/ajax/chapters/"
      ajax_response2 = http.post(ajax_url, headers: xhr_headers)

      if ajax_response2.status == 200 && ajax_response2.body.present?
        doc = Nokogiri::HTML(ajax_response2.body)
        result = parse_chapters(doc)
        return result if result.any?
      end

      # Final fallback: parse inline chapters from series page
      response = http.get(url)
      return [] unless response.status == 200
      ...
      Rails.logger.error "[MangaRead] Chapters error: #{e.message}"
      []
    end
    ...
    def pages(chapter_url)
      ...
      Rails.logger.error "[MangaRead] Pages error: #{e.message}"
      []
    end
    ...
    def browse_html_fallback(sort, page)
      ...
      Rails.logger.error "[MangaRead] Browse HTML fallback error: #{e.message}"
      []
    end
    ...
    # Parse chapters from Madara series page
    ...
    # Extract a post ID placeholder from URL for AJAX requests
    # The manga_get_chapters action needs the WP post ID, but we use the slug
    # as a reasonable fallback since the AJAX endpoint often accepts it
    ...
    # Parse Madara date formats (MM/dd/yy or relative like "3 hours ago")
