...
# MangaNato adapter (natomanga.com)
# Reference: https://github.com/keiyoushi/extensions-source/tree/main/src/en/mangakakalot
#
# MangaBox family site (sister site to MangaKakalot). Uses the same platform:
# - JSON API for chapter lists
# - HTML scraping for search/series/browse
...
module Scrapers
  ...
  class Adapter < Scrapers::BaseAdapter
    ...
    # (MangaBox sites use a JS function called change_alias that normalizes queries)
    VIETNAMESE_MAP = {
      "\u00E0" => "a", "\u00E1" => "a", "\u1EA1" => "a", "\u1EA3" => "a", "\u00E3" => "a",
    ...
    def search(query)
      ...
      items.filter_map do |item|
        ...
        # Extract author from metadata span if present
        author = item.at_css("span.story_item_author, .author")&.text
        author = author&.sub(/\Aauthor\(?s?\)?\s*:\s*/i, "")&.strip
      ...
      Rails.logger.error "[MangaNato] Search error: #{e.message}"
      []
    end
    ...
    def series(id_or_url)
      ...
      Rails.logger.error "[MangaNato] Series error: #{e.message}"
      nil
    end
    ...
    def chapters(series_url)
      ...
      Rails.logger.error "[MangaNato] Chapters error: #{e.message}"
      []
    end
    ...
    def pages(chapter_url)
      ...
      Rails.logger.error "[MangaNato] Pages error: #{e.message}"
      []
    end
    ...
    def browse(sort: "latest", page: 1, limit: 20)
      ...
      Rails.logger.error "[MangaNato] Browse error: #{e.message}"
      []
    end
    ...
    def extract_table_value(doc, label)
      # Pattern 1: li containing label text with link values
      li = doc.css("li, td").find { |el| el.text.downcase.include?(label.to_s) && el.at_css("a") }
      if li
        value = li.css("a").map { |a| a.text.strip }.join(", ")
        return value if value.present?
      end

      # Pattern 2: table row with label and value cells
